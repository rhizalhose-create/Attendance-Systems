...
func GenerateStudentQRCode(studentID, email, firstName, lastName, course string) (string, error) {
    qrData := fmt.Sprintf("STUDENT_ID|%s|%s|%s|%s|%s|%d", 
        studentID, email, firstName, lastName, course, time.Now().Unix())

    return generateQRCodeBase64(qrData)
}

// GenerateEventQRCode generates QR code for specific event
func GenerateEventQRCode(studentID, eventType, eventName string, eventID uint) (string, error) {
    qrData := fmt.Sprintf("EVENT|%s|%s|%s|%d|%d", 
        studentID, eventType, eventName, eventID, time.Now().Unix())

    return generateQRCodeBase64(qrData)
}

// GenerateCustomQRCode generates QR code with custom type
func GenerateCustomQRCode(studentID, qrCodeType, customData string) (string, error) {
    qrData := fmt.Sprintf("%s|%s|%s|%d", 
        qrCodeType, studentID, customData, time.Now().Unix())

    return generateQRCodeBase64(qrData)
}

// GenerateQRCodeFile generates QR code and saves as file
func GenerateQRCodeFile(studentID, qrCodeType, data string) (string, error) {
    qrData := fmt.Sprintf("%s|%s|%s|%d", qrCodeType, studentID, data, time.Now().Unix())
    ...
    // Generate file path
    fileName := fmt.Sprintf("qr_%s_%s.png", studentID, qrCodeType)
